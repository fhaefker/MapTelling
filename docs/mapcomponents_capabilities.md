# MapComponents Capabilities & Reference (Canonical Copy for MapTelling)

This file is the authoritative rule & knowledge base derived from the external `MAPCOMPONENTS_CAPABILITIES.md` (snapshot date 2025-09-04). All future development tasks in this repository must comply with the conventions, components, hooks, workflows, and constraints described herein.

> Maintenance: When upstream capabilities change, update this file and adjust the API hash and affected sections. Commit message pattern: `docs(capabilities): sync api hash <old> -> <new>`.

---

<!-- BEGIN ORIGINAL CONTENT SNAPSHOT -->
...
1. Scope: All code, architectural decisions, component designs, performance considerations, and naming conventions in this repository MUST align with the rules in this document.
2. Deprecated Props: Never introduce new usages of deprecated props (`paint`, `layout` on `MlGeoJsonLayer`). Use `options` nesting pattern.
```
Verknüpfen in Projekt:
```
3. Multiple Maps: Always assign explicit `mapId` per map and for every dependent hook/component where ambiguity could arise.
cd ../MapTelling/maptelling-react
4. Cleanup Discipline: Every imperative interaction (events, sources, layers outside declarative components) must bind through wrapper methods or dedicated hooks to guarantee cleanup.
5. Protocol Extensions: Custom data ingestion must use `useAddProtocol` with security review (size limits, schema validation).
```
...
<!-- END ORIGINAL CONTENT SNAPSHOT -->
6. Performance Baseline: Avoid DOM markers for > 1000 points—prefer Circle layers. Large GeoJSON (>5MB) requires justification (tiling plan or simplification documented).
7. API Drift Monitoring: If upgrading `@mapcomponents/react-maplibre`, run (future) extraction script and update `apiHash` + note differences in CHANGELOG.
8. Theming: Style switching must preserve existing sources when possible; minimize unnecessary `setStyle` calls (reference equality check required).
9. Testing: New components require at minimum: render test, storybook story, and (if interactive) event interaction test.
10. Security: Popups must sanitize or escape all feature property values before rendering.

## Planned Automation

A future script (`scripts/extract-mapcomponents-api.ts`) will:
- Parse type definitions of the dependency.
- Emit JSON + Markdown (placed under `docs/generated/`).
- Validate `apiHash` vs computed hash; fail CI if mismatch unless commit message includes `api-drift: allow`.

Until implemented, manual verification is required.

---

(End of canonical policy overlay)

---
## 61. PWA & Offline Architektur (Aus org Repos konsolidiert)
Quelle: `pwa-demo` Repo.

Ziele:
- Offline-Zugriff auf Karten & Daten (Service Worker + Caching Strategien).
- Schnelle Wiederinitalisierung (Warm Cache) für wiederkehrende Nutzer.

Kernkomponenten / Artefakte:
- Service Worker (generiert oder manuell) – kümmert sich um precache dynamische Tiles, mc_meta, App Shell.
- `mc_meta.json` (Meta Konfigurationsdatei – siehe Abschnitt 69).
- Datenverarbeitungs-Pipeline via `yarn processData` (Generiert: Background Tiles, Data Tiles, Geometrie, Suchindizes).
- Docker Image `mc_pwa_data_processing` ermöglicht deterministische Datenaufbereitung.

Architektur-Skizze (Phasen):
1. Datenquelle (z.B. GeoPackage, Shapefile, CSV) → Konvertierung (Node Skripte im pwa-demo) → Normalisierte GeoJSON/Vector Tile Kacheln.
2. Preprocessing: Build Suchindex (z.B. vollständiger Text- oder Prefix-Index), Simplifikation / Generalisierung.
3. Deployment: Static Hosting + Service Worker precache der Kern-Assets + Laufzeit-Fetch für Tiles mit Cache-First-Fallback.
4. Offline Nutzung: App Shell + zuletzt verwendete Styles + Kennzahlen aus IndexedDB / Cache Storage.

Empfohlene Patterns (MapTelling):
- Trenne build-time Datenpipeline (Docker) von runtime App Code; ordner `data_pipeline/`.
- Versioniere Datenartefakte (z.B. `tiles_v{ts}/`).
- Prüfsummen (SHA256) für mc_meta Einträge zur Cache-Invalidierung.

Risiken & Mitigation:
| Risiko | Wirkung | Lösung |
|-------|---------|--------|
| Überfüllter Cache | Speicherknappheit mobil | LRU Cleanup & Max-Größe Schwelle |
| Invalide Suchindizes | Fehlende Treffer | Versionsstempel + Integrity Check |
| Große Erstinstallation | Langsamer Erststart | On-Demand Tile Loading / gestaffeltes Prefetch |

## 62. Datenverarbeitungs-Pipeline (processData Command)
Extrahiert aus `pwa-demo`.

Konzeptionelle Schritte (angenommen):
1. Rohdaten Einsammeln (`/assets` oder extern).
2. Normalisierung (Projektion vereinheitlichen, Properties filtern).
3. Generalisierung (Mehrskalige Vereinfachung; Douglas-Peucker oder Visvalingam-Algorithmen – TODO: Tooling festlegen).
4. Tile Generierung (Vector / Raster) – Empfehlung: `tippecanoe` für Vector Tiles, optional MBTiles Zwischenschritt.
5. Index-Aufbau (Suche): Inverted Index JSON + optional Fuse.js konformer Index.
6. Manifest (mc_meta.json) Aktualisierung (Version, Checksums, Build Timestamp, Schema Version).

Bewertungskriterien:
- Determinismus (gleicher Input => gleicher Hash) für Caching.
- Reproduzierbarkeit (Docker Umgebung pinned Versions).
- Skalierbarkeit (Parallelisierung nach Layern).

Empfohlene Erweiterung (MapTelling):
- Skript `scripts/process-data.ts` mit modularem Step Runner.
- Konfigurationsdatei `data.config.json` (Liste Layer, Quelle, Ziel Zoom-Bereiche, Simplify Toleranzen).
- Output Ordnerstruktur: `public/data/{layer}/{z}/{x}/{y}.pbf` oder `.../geojson` für fallback.

## 63. QGIS Export Plugin (QGIS-to-MapComponents) Tiefergehend
Zweck: Automatisiert QGIS Projekt → MapComponents App.

Feature-Zusammenfassung (aus README):
- Export `.qgs` / `.qgz` → Web App.
- Reproduktion: Vector Layer, WMS Services.
- Layer Konfiguration (Style, Sichtbarkeit) als JSON.
- Projektdaten (Titel, Layernamen) übernommen.
- Template wählbar (mehrere im Plugin `templates/`).

Export-Pipeline (abstrahiert):
1. Parsen QGIS Projekt (PyQGIS API) → interne Layer Strukturen.
2. Generieren Konfig JSON (`layers/*.json`) + evtl. Style Transfer (Symbologie Mapping: QGIS → MapLibre Paint/Layout – zukünftiger Deep-Mapping Task).
3. Kopieren React Template → Zielverzeichnis.
4. Einfügen generierter Config + Option Start Dev Server Hinweis.

Integrationsideen für MapTelling:
- Importpfad in eigener CLI: `maptelling import-qgis --project my.qgz --template base` (ruft Plugin Logik Headless oder repliziert Algorithmus).
- Validierung: Layer Count Limit, Unsupported Symbol Styles protokollieren.

Gap Offene Punkte (nicht im aktuellen Capabilities Doc vor Integration):
- Symbologie Mapping Tabelle.
- Fehlerbehandlung (fehlende Projektion / CRS Konflikt).
- Performance bei großen Layern (Chunk Export + On-Demand Laden?).

## 64. Template Repository (mapcomponents/template) Detailergänzungen
Bereits erwähnt (Section 3), fehlende Spezifika ergänzt:
- Dockerfile vorhanden → Basis für containerisierte Dev Umgebung (Node >=16). Empfehlung: override Node Version für LTS (18/20).
- Minimaler `vite.config.ts` – Hot Module Replacement + TypeScript Pfad Aliase optional ergänzen.
- CI Empfehlung: Lint + Type Check + Build Matrix (14/16/18) bevor Release.
- Production Build: `yarn build` → deploy `dist/` (Netlify/Vercel).

## 65. Beispiel Apps (react-map-components-apps)
Verzeichnis-Layer (Apps): `multi_tab`, `pointcloud`, `powerplants`, `running_contest`, `webGis`.

Pattern-Extraktion:
- Jede App isolierbar (eigene `package.json` / oder gemeinsamer Root? – Analyse TODO falls nicht bekannt).
- Dient als Referenz für: Multi App Navigation, domain-spezifische Visualisierung (Pointcloud / Energie / Wettlauf / klassisches WebGIS).

Best Practices aus Apps ableiten:
- Domain-spezifische Komponenten in `components/domain/` strukturieren.
- Gemeinsame Utility (Formatters, Color Scales) extrahieren in Shared Paket oder Monorepo Workspace.
- Thumbnail Assets (`assets/thumbnails`) für Katalog / Showcase generieren.

## 66. Tutorial / Webinar (webinar-2025) Step Struktur
Pattern: Schrittweises Verzeichnis `Step1...StepN`, jede Stufe vollständige lauffähige App + README (Delta Erklärung).

Empfohlene interne Nutzung:
- Für komplexe Features (z.B. Terrain + Offline + QGIS Import) analoges "evolutionary steps" Verzeichnis `tutorials/featureX/Step1..`.
- Automatischer Diff Generator (git diff StepN-1 StepN) in README anzeigbar.

## 67. Developer Handbook (developer-manual)
Tooling: Docusaurus 2 (MDX + React) für Dev-Dokumentation.

Relevanz für Capabilities Doc:
- Könnte als Upstream Ort für Abschnitte 61–72 dienen (Contribution upstream optional).
- Build Steps (npm start / npm build / deploy) bereits Standard – keine zusätzlichen Regeln nötig.

MapTelling Empfehlung:
- Falls umfangreiche interne Erweiterungen entstehen → eigenen Docusaurus Teil `docs/` statt monolithisches Markdown.

## 68. Catalogue vs Lab Unterscheidung
Catalogue (`react-map-components-catalogue`): Produktiv-präsentationsorientiert (Showcase / Interaktive Gallery). Ziel: Stabile Beispiele.
Lab (`react-map-components-maplibre-lab`): Experimenteller Raum (Prototyping, zukünftige Komponenten). Ziel: Schnelles Iterieren, potentielle API-Vorstufen.

Governance Pattern:
| Stufe | Ort | Stabilitätsgrad | Migrationspfad |
|-------|-----|-----------------|----------------|
| Idee/Experiment | Lab | Niedrig | Evaluieren → Hardening |
| Kuratierter Showcase | Catalogue | Mittel-Hoch | Upstream Maintenance |
| Prod-Ready Component | Core Library | Hoch | SemVer Release |

## 69. `mc_meta.json` (Meta Manifest) – Vorläufige Schema-Hypothese
Kontext: PWA Demo & Apps.

Zweck: Beschreibt verfügbare Layer, Datensätze, Versionen, evtl. Indizes und Ressourcenpfade für Client Bootstrapping.

Vorschlag Schema (Erweitern nach tatsächlicher Extraktion):
```jsonc
{
  "schemaVersion": 1,
  "generatedAt": "2025-09-05T12:00:00Z",
  "dataVersion": "2025.09.05",
  "layers": [
    { "id": "powerplants", "type": "vector", "format": "pbf", "minzoom": 0, "maxzoom": 12, "checksum": "sha256-...", "bytes": 1234567 },
    { "id": "base_raster", "type": "raster", "format": "png", "tileSize": 256 }
  ],
  "searchIndex": { "path": "data/search/index.json", "checksum": "sha256-..." },
  "assets": [ { "path": "sprites/sprite.json", "checksum": "sha256-..." } ],
  "offline": { "precache": ["/","/index.html","/mc_meta.json"], "strategy": "stale-while-revalidate" }
}
```

Follow-Up Task: Tatsächliches Schema aus vorhandenen Repos extrahieren & VALIDATE → Abschnitt updaten.

## 70. Governance & Contribution Richtlinien (Konsolidiert)
Quellen: `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md` (Core Library Repo)

Kernaussagen:
- Verhaltenskodex (Contributor Covenant) gilt für alle Repos.
- Contributions: Fork → Branch (konventionelles Branch Schema) → Tests/Story → PR.
- Erwartete Qualität: Lint + Test Passing + Storybook Aktualisierung.
- SemVer Einhaltung für Breaking Changes (PR kennzeichnen).

Ergänzung MapTelling Lokal:
- Interne PR Vorlage `PULL_REQUEST_TEMPLATE.md` (TODO) mit Feldern: Motivation, Veränderter API Surface, Performance-Impact, Migrationshinweise.

## 71. Repository Rollen Matrix
| Repo | Rolle | Stabilität | Import in Capabilities Doc (Status) |
|------|------|-----------|--------------------------------------|
| react-map-components-maplibre | Core Library | Hoch | Abgedeckt (1–60) |
| template | Bootstrap Template | Hoch | Abschnitt 3 + 64 (OK) |
| react-map-components-catalogue | Showcase | Mittel | Ergänzt 68 |
| react-map-components-maplibre-lab | Experiment | Niedrig | Ergänzt 68 |
| react-map-components-apps | Beispiele | Mittel | Abschnitt 65 |
| webinar-2025 | Tutorial/Schulung | Mittel | Abschnitt 66 |
| pwa-demo | Offline/PWA Referenz | Mittel | Abschnitte 61–62 + 69 |
| developer-manual | Dev Dokumentation (Docusaurus) | Mittel | Abschnitt 67 |
| QGIS-to-MapComponents | Desktop Export Integration | Mittel | Abschnitt 63 |
| mapcomponents (monorepo) | Aggregation / Zukunft | Variabel | Liste (Sektion 5) – evtl. spätere Monorepo Richtlinien (TODO) |

Fehlende evtl. kleinere Repos (falls existieren) → Nachführung bei Sichtung.

## 72. Offene Gaps & Nächste Schritte (Meta)
| Gap | Beschreibung | Priorität | Maßnahme |
|-----|--------------|----------|----------|
| mc_meta echtes Schema | Hypothese vs Realität | Hoch | Automatisches Schema Extraction Skript schreiben |
| Prozess `processData` Details | Unbekannte interne Schritte | Mittel | Quellcode Analyse & Dokumentation als Unterabschnitt 62.X |
| QGIS Symbol Mapping | Nicht definiert | Mittel | Mapping Tabelle QGIS Symbol → MapLibre Paint erstellen |
| Offline Cache Strategie Feinjustierung | Statisches Pattern | Mittel | Audit Real-World & TTL/Quota Mgmt ergänzen |
| Security Offline (SW Integrity) | Nicht dokumentiert | Mittel | Subresource Integrity / Hash Pinning notieren |
| Monorepo Governance | mapcomponents root | Niedrig | Falls aktiv genutzt: Arbeitsbereich Struktur aufnehmen |

Integration Policy Update:
Ab jetzt gelten Abschnitte 61–72 ebenfalls als Kanon. Neue Aufgaben müssen bei Relevanz a) PWA, b) QGIS Export, c) Datenpipeline diese Referenzen beachten.

---
Ende Addendum (Org Deep Analysis). Bei zukünftigen Upgrades zuerst Tabelle in Abschnitt 71 & Gaps (72) prüfen.
